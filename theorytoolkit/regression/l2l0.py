""" L2L0 regularization least square solver. """
...
class L2L0Estimator(BaseEstimator):
    ...
    def fit(self,feature_matrix, target_vector, sample_weight=None, hierarchy=None,\
        ...
        Inputs:
            ...
            mu(1D array-like of length 1 or None):
                A tuple of (mu0,mu2). L1L0 loss function is:
                L = ||Xw-y||^2 + mu0 * ||w||_0 + mu2 * ||w||^2
        ...
        if mu is None or len(mu)!=2:
            ...
            if mu[1]<=np.power(10,float(log_mu_ranges[1][0])):
                warnings.warn("Minimun allowed mu_2 taken!")
            if mu[1]>=np.power(10,float(log_mu_ranges[1][1])):
                warnings.warn("Maximum allowed mu_2 taken!")
    ...
    def _solve(self,feature_matrix,target_vector,hierarchy=None,mu=[0,0],\
               M=100,tlimit=300):
        """
        Called by super().fit. Solves an L2L0 model with hierarchy constraints.
        Inputs:
            M(float):
        ...
        constraints = [0<=z0, z0<=1, M*z0>=w, M*z0>=-w]
        ...
        # Cost function
        L = cp.sum_squares(A@w-b)+mu[0]*cp.sum(z0)+mu[1]*cp.sum_squares(w)
